...
getDEIso <- function(seurat, gtf, subset_ident, cluster_column,
                     min.pct = 0.25, rate_cutoff = 0.1,
                     multi_cutoff = 0.2, ratio_range = 0.05,
                     pair_cutoff = c(-0.9,0.9), ratio_difference = 0.1) {
  
  if (is.null(multi_cutoff) && is.null(pair_cutoff)) {
    stop(
      "Both multi_cutoff and pair_cutoff are NULL.\n",
  ...
  # ------------------
  # multi-DEIso（optional）
  # ------------------
  if (!is.null(multi_cutoff)) {
    result$multi <- identify_multi_DiffIso(
      markers,
  ...
  # pair-DEIso（optional）
  # ------------------
  if (!is.null(pair_cutoff)) {
    result$pair <- identify_pair_DiffIso(
      markers,
      subset_ident,
